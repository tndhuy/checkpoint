...
2. Frontmatter has all 8 mandated keys (`type: report`, `created`, `profile`, `scope`, `role`, `project`, `branch`, `related_checkpoint`) with real, non-placeholder values. `branch: ""` is a genuine `Unknown`-equivalent — the scratch directory has no `.git`, and the agent left it empty rather than inventing a branch name.
