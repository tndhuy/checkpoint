...
Read the chosen checkpoint in full. Before taking action, report concise answers to these questions, in the language resolved per `../checkpoint/references/scope-and-role.md`'s Language section (persisted `language` wins over the current request's language once set; not the language the checkpoint document happens to be stored in):
